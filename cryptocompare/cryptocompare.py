...
def _query_cryptocompare(url: str, errorCheck: bool = True) -> Optional[Dict]:
    ...
    try:
        ...
        if apikey:
            ...
            logger.warning(
                'No API key for cryptocompare found, querying without (might bump into limits).'
            )
...
def get_price(coin: str, curr: str = CURR, full: bool = False) -> Optional[Dict]:
    ...
    if isinstance(coin, list):
        return _query_cryptocompare(
            _URL_PRICE_MULTI.format(_format_parameter(coin), _format_parameter(curr))
        )
...
def get_historical_price_day(
    ...
    response = _query_cryptocompare(
        _URL_HIST_PRICE_DAY.format(
            coin, _format_parameter(curr), limit, exchange, _format_timestamp(toTs)
        )
    )
    if response:
...
def get_avg(coin: str, curr: str = CURR, exchange: str = 'CCCAGG') -> Optional[Dict]:
    ...
    """
    response = _query_cryptocompare(
        _URL_AVG.format(coin, curr, _format_parameter(exchange))
    )
    if response:
        return response['RAW']
    return None
